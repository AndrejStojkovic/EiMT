...
const DeleteHostModal = ({ open, hostName, deleting, errorMessage, onClose, onConfirm }: DeleteHostModalProps) => {
  return (
    <Dialog open={open} onClose={deleting ? undefined : onClose} fullWidth maxWidth='xs'>
      <DialogTitle sx={{ pb: 1 }}>
        <Stack direction='row' spacing={1} sx={{ alignItems: 'center' }}>
          <DeleteForeverRoundedIcon color='error' />
          <Typography variant='h6'>Delete host</Typography>
        </Stack>
      </DialogTitle>
      <DialogContent>
        <Stack spacing={1.5}>
          ...
          <Typography variant='body1'>
            Are you sure you want to permanently remove <strong>{hostName}</strong>?
          </Typography>
