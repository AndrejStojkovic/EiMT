...
interface UserCardProps {
  ...
  onEdit?: (user: RegisterResponse) => void;
  onDelete?: (user: RegisterResponse) => void;
  isAdmin: boolean;
}
...
const UserCard = ({ user, onEdit, onDelete, isAdmin }: UserCardProps) => {
  ...
  const detailPath = `/users/${user.username}`;
    ...
    >
      ...
      >
        <Stack spacing={0.25} sx={{ alignItems: 'center' }}>
          <PersonIcon sx={{ fontSize: 52, color: alpha('#fff', 0.95) }} aria-hidden />
          <Typography variant='subtitle2' sx={{ color: alpha('#fff', 0.9), letterSpacing: '0.05em' }}>
            USER
          </Typography>
        </Stack>
      </CardMedia>
        ...
        <Stack direction='row' spacing={1} sx={{ mb: 1.25, justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          {isAdmin && (
            <IconButton
              ...
              aria-controls={menuOpen ? 'user-card-menu' : undefined}
              aria-haspopup='true'
      ...
      {isAdmin && (
        <Menu
          ...
          slotProps={{
            list: { dense: true, 'aria-labelledby': 'user-card-menu' },
          }}
        >
          <MenuItem
            onClick={() => {
